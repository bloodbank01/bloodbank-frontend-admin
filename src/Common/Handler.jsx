...
export const exportData = (data, format, filename = 'data') => {
  ...
  switch (format) {
    ...
    case 'txt': {
      const txt = data.map(item => JSON.stringify(item)).join('\n');
...
export const getRoute = (type) => {
  ...
  let doctor = localStorage.getItem('is_doctor')
...
export const convertMobileFormat = (phone) => {
  if (phone) {
    const digits = phone?.replace(/\D/g, '');

    // If it's 11 digits and starts with '0' or '1' or country code, remove the first digit
    const cleaned = digits.length === 11 && /^[01]/.test(digits) ? digits.slice(1) : digits;
