...
const EmailSent = ({ text = null, subText = null, setValue, navigateTo }) => {
    ...
    return (
        <div className={`position-fixed top-0 start-0 z-50 w-100 h-100 d-flex justify-content-center align-items-center px-3 modal-overlay ${closing ? 'hide' : ''}`} style={{ backgroundColor: 'rgba(0,0,0,0.5)', zIndex: '9999' }}>
            <div className={`bg-white position-relative overflow-hidden rounded-4 modal-content ${closing ? 'hide' : ''}`} style={{ width: '750px', height: '364px' }}>
                ...
                <div className="position-absolute top-0 start-0 w-100 d-flex justify-content-center py-5">
                    <div className="d-flex flex-column align-items-center justify-content-center h-auto text-center">
                        ...
                        <h3 className="mt-4 fw-semibold" style={{ fontSize: '28px' }}>{text ? text : 'Registration Email has been sent to your email!'}</h3>
                        <p className="text-muted my-1">{subText ? subText : 'Please check your inbox and verify.'}</p>
